...
#[command(name = "ysc")]
#[command(version)]
#[command(about = "ysc CLI: runner, REPL, and code formatter.")]
struct Cli {
    #[command(subcommand)]
...
fn main() {
    ...
    let result = match (cli.command, cli.eval, cli.file) {
        ...
        (Some(Commands::Run { file }), _, _) => run::run_file(&file).map_err(|e| {
        ...
        (None, Some(code), _) => run::run_source(&code).map_err(|e| report_error(e, &code)),
        (None, None, Some(file)) => run::run_file(&file).map_err(|e| {
            let source = std::fs::read_to_string(&file).unwrap_or_default();
            report_error(e, &source)
        }),
    };
