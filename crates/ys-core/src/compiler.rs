...
impl Loc {
    ...
    /// Convert to `(usize, usize)` for [`JitError`] construction.
    pub fn as_error_pos(self) -> (usize, usize) {
...
impl Value {
    /// The nil/zero value — used as empty/void default throughout the VM.
    ...
    #[inline(always)]
    pub fn pool(id: u32) -> Self {
        Self(QNAN | TAG_POOL | (id as u64))
    }

    /// Create a failure value tagged with a failure-type name ID.
    #[inline(always)]
    pub fn failure(name_id: u32) -> Self {
        Self(QNAN | TAG_FAILURE | (name_id as u64))
    }
...
pub enum Instruction {
    ...
    //  Arithmetic
    /// Add numbers (fast path — skips string concatenation, but falls
    /// through to the generic path when operands aren't plain f64).
    AddNum {
        dst: usize,
        lhs: usize,
        rhs: usize,
    ...
    /// Add pure f64 numbers — no failure/string checks. Emitted when the
    /// compiler can prove both operands are numeric at compile time.
    AddNumFast {
        dst: usize,
        lhs: usize,
        rhs: usize,
    ...
    Increment(usize),
    /// Atomic decrement of a local register.
    Decrement(usize),
    /// Atomic increment of a global variable slot.
    IncrementGlobal(usize),
    /// Atomic decrement of a global variable slot.
