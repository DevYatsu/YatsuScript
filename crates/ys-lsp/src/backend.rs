...
use crate::analysis::{AnalysisResults, LspTokenVariant, analyze_source};
...
impl YatsuBackend {
    ...
    async fn update_document(&self, uri: Url, source: String) {
        ...
        self.documents
            .insert(uri.clone(), (results.clone(), source));

        self.client
            .publish_diagnostics(uri, results.diagnostics.clone(), None)
            .await;
    }
...
impl LanguageServer for YatsuBackend {
    ...
    async fn initialized(&self, _: InitializedParams) {
        self.client
            .log_message(MessageType::INFO, "ysc LSP server initialized")
            .await;
    }
