...
/// Object fields with a lazy-computed sorted-key cache for iteration.
pub struct ObjectData {
    pub map: rustc_hash::FxHashMap<u32, Value>,
    pub sorted_keys: Option<Vec<u32>>,
}

impl ObjectData {
    pub fn new(map: rustc_hash::FxHashMap<u32, Value>) -> Self {
        Self {
            map,
            sorted_keys: None,
        }
    ...
    /// Return the sorted key list, computing it lazily on first call.
    pub fn sorted_keys(&mut self) -> &[u32] {
        self.sorted_keys.get_or_insert_with(|| {
...
impl ManagedObject {
    // ── Enum accessors ────────────────────────────────────────────────
    // Follow the `Value::as_*()` pattern: returns `Option` so callers can
    // use `if let Some(x) = obj.as_list()` instead of a full match.

    pub fn as_string(&self) -> Option<&Arc<str>> {
        match self {
            ManagedObject::String(s) => Some(s),
            _ => None,
        }
    }
    pub fn as_list(&self) -> Option<&Vec<Value>> {
        match self {
            ManagedObject::List(v) => Some(v),
            _ => None,
        }
    }
    ...
    pub fn as_object(&self) -> Option<&rustc_hash::FxHashMap<u32, Value>> {
        match self {
            ManagedObject::Object(d) => Some(&d.map),
            _ => None,
        }
    }
    pub fn as_object_mut(&mut self) -> Option<&mut rustc_hash::FxHashMap<u32, Value>> {
        match self {
            ManagedObject::Object(d) => Some(&mut d.map),
            _ => None,
        }
    }
    pub fn as_range(&self) -> Option<(f64, f64, f64)> {
        match self {
            ManagedObject::Range { start, end, step } => Some((*start, *end, *step)),
            _ => None,
        }
    }
    pub fn as_promise(&self) -> Option<&crate::vm::PromiseState> {
        match self {
            ManagedObject::Promise(p) => Some(p),
            _ => None,
        }
    }
    pub fn as_promise_mut(&mut self) -> Option<&mut crate::vm::PromiseState> {
        match self {
            ManagedObject::Promise(p) => Some(p),
            _ => None,
        }
    ...
    pub fn as_closure(&self) -> Option<&Closure> {
        match self {
            ManagedObject::Closure(c) => Some(c),
            _ => None,
        }
    }
    pub fn as_bound_method(&self) -> Option<(&Value, u32)> {
        match self {
            ManagedObject::BoundMethod { receiver, name_id } => Some((receiver, *name_id)),
            _ => None,
        }
    }

    pub fn is_string(&self) -> bool {
        matches!(self, ManagedObject::String(_))
    }
    pub fn is_list(&self) -> bool {
        matches!(self, ManagedObject::List(_))
    }
    pub fn is_object(&self) -> bool {
        matches!(self, ManagedObject::Object(_))
    }
    pub fn is_promise(&self) -> bool {
        matches!(self, ManagedObject::Promise(_))
    }
    ...
    pub fn visit_children<F: FnMut(u32)>(&self, mut f: F) {
        match self {
            ...
            ManagedObject::BoundMethod { receiver, .. } => {
                if let Some(id) = receiver.as_obj_id() {
                    f(id);
                }
            }
            ...
            ManagedObject::Promise(state) => {
                match state {
                    ...
                    crate::vm::PromiseState::Compound {
                        sub_promises,
                        results,
                        continuation,
                    } => {
                        for id in sub_promises.iter().flatten() {
                            f(*id);
                        }
...
impl Heap {
    ...
    /// Walk the worklist, marking live objects with the current GC ID.
    /// Used by both [`major_gc`] and [`minor_gc`].
    fn drain_worklist(gc_id: u32, worklist: &mut Vec<u32>, objects: &mut [Option<HeapObject>]) {
    ...
    pub fn major_gc(&self, gc_id: u32, ctx: &Context) {
        ...
        Self::drain_worklist(gc_id, &mut worklist, objects);
        ...
        let new_from_old: Vec<u32> = meta
            ...
            .filter(|&&id| {
                objects
                    ...
                    .and_then(|s| s.as_ref())
                    .map(|o| {
                        o.generation == Generation::Tenured && self.points_to_nursery(o, objects)
                    })
                    .unwrap_or(false)
            })
            .copied()
