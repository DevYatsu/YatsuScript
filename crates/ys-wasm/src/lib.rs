...
pub fn eval(source: &str) -> JsValue {
    ...
    let result = std::panic::catch_unwind(|| match ys_core::codegen::Codegen::compile(source) {
...
pub fn disassemble(source: &str) -> JsValue {
    let out = js_sys::Object::new();
    let result = std::panic::catch_unwind(|| ys_core::codegen::Codegen::compile(source));
    match result {
        Ok(Ok(program)) => {
            js_sys::Reflect::set(&out, &"success".into(), &JsValue::from(true)).ok();
            ...
            for (fi, func) in program.functions.iter().enumerate() {
                ...
                js_sys::Reflect::set(
                    &f,
                    &"params".into(),
                    &JsValue::from(func.params_count as u32),
                )
                .ok();
                js_sys::Reflect::set(
                    &f,
                    &"locals".into(),
                    &JsValue::from(func.locals_count as u32),
                )
                .ok();
